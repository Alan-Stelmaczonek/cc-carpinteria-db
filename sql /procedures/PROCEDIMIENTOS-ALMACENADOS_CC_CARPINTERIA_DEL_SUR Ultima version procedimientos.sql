...
CREATE PROCEDURE SP_Insert_Cliente 
    ...
    @doc_tipo NVARCHAR(20) ,
    @doc_numero NVARCHAR(20) ,
...
EXEC SP_Insert_Cliente @tipo = 'Particular', @nombre = 'Juan', @apellido = 'Stelmaczonek', @razon_social = 'Muebles Alan', @doc_tipo = 'DNI', @doc_numero = '46683669', @telefono = '1137558221', @email = 'titi.alan05az@gmail.com';
GO
...
CREATE PROCEDURE SP_descuentos_en_pedidos_de_cliente
  @id_cliente INT,
...
EXEC SP_descuentos_en_pedidos_de_cliente @id_cliente = 10000;           -- solo con descuento
EXEC SP_descuentos_en_pedidos_de_cliente @id_cliente = 10000, @solo_con_descuento = 0;  -- todo
GO
...
BEGIN
  ...
  JOIN Producto p ON p.id_producto = df.id_producto
  JOIN Material m ON m.id_material = df.id_material
  WHERE df.id_producto = @id_producto;
END
GO

EXEC sp_materiales_por_producto @id_producto = 3;
...
SELECT FN_TotalNetoPedido(1) AS Total_Neto;
...
BEGIN
    SELECT 
        ...
        p.total_bruto,
        p.descuento_total,
        FN_TotalNetoPedido(p.id_pedido) AS total_neto
    FROM Pedido p
